...
export const RiskReturnMap = () => {
  ...
  const handlePointClick = () => {
    if (domain) setDomain(null);
      ...
      // Extra tight domain for "Zoom" mode
    ...
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Default range: Risk 0-80, Return 0-40. Click chart to auto-scale.</p>
        </div>
        
        <div className="flex gap-2 w-full max-w-2xl">
          <input type="text" value={tickerInput} onChange={(e) => setTickerInput(e.target.value)} className="flex-1 bg-card border border-input rounded-lg px-4 py-2 text-foreground" placeholder="SPY QQQ DIA..." />
      ...
      {data.length > 0 && (
        <div className="h-[600px] bg-card border border-border rounded-xl p-4 shadow-sm cursor-pointer relative overflow-hidden" onClick={handlePointClick}>
            ...
            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              <Tooltip cursor={{ strokeDasharray: '3 3' }} content={({ active, payload }) => {
                  if (active && payload?.length) {
                    ...
                    return <div className="bg-popover border border-border p-3 rounded-lg shadow-xl"><p className="font-bold text-foreground">{d.ticker}</p><p className="text-sm text-green-500">Ret: {d.return.toFixed(1)}%</p><p className="text-sm text-red-400">Risk: {d.risk.toFixed(1)}%</p></div>;
                  } return null;
                }} />
